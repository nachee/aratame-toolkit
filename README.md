...
`model` is required for planning and uncovered-case exploration, but not for reviewing or running an entirely linked plan. `baseUrl` is required and must match the reviewed plan exactly. It cannot contain credentials, a query, or a fragment. Keep the separate Cloud runner config out of this local config; unknown fields are rejected.
...
1. Run linked Playwright tests first, serially, without retries. A failing existing test is never rewritten to pass; review it before uncovered cases can proceed.
...
The Cloud worker receives approved jobs, sends reports/reviews to Cloud, and sends browser model requests through Cloud's configured provider roles. Raw browser artifacts remain local, while report logs and evidence descriptions can be included in Cloud reports. Its config can include the same optional local execution settings; Cloud supplies the model configuration, so a local `model` is not used for Cloud execution. The worker never turns a standalone plan into a hosted submission automatically.

Feature-staged Cloud runs require a worker advertising `script-revisions-v1`. Updated workers advertise this capability when claiming work, so existing enrollments need no replacement. Cloud assigns immutable plan/case revisions and snapshots approved structured coverage for that run; feature execution does not publish cases or automation into the shared baseline.

Before executing each staged spec, the worker retains an exclusive read-only copy under the run's local artifact directory and binds its SHA-256, project-relative path and optional tag to the active Cloud assignment. Executed results report that digest; changed source bytes or a different binding are rejected. Original spec paths remain in use so relative imports and project configuration still work. This identifies the entry spec file, not its imports, fixtures, configuration, complete checkout, or an adversarial filesystem; raw source copies and browser artifacts are not uploaded.
