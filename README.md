...
Omit `--knowledge` or `--case` to select all entries of that kind; repeat either flag to select explicit IDs. Importing definitions requires 1–40 selected cases and at most 21 selected pages. A KB-only manifest is valid: to draft new cases, provide `--requirements requirements.txt` with the same revision flags and an explicit BYOK model. Selected pages and existing definitions enter the existing bounded draft/critique/merge process as evidence, not self-authorizing tests or original requirements. Ordinary Markdown can also be used as an explicitly named `--context` file without Git; that mode makes no published-revision claim.
...
Manifest version 2 (`kind: "aratame-artifacts"`) lists explicit relative files with SHA-256, `kind` (`knowledge`, `conventions`, `provenance`, `script`, `fixture`, `test-definition`, `source-snapshot`), and generated/authored `origin`. It retains knowledge IDs, surfaces, citations, source versions, gaps and history; case definitions and script hashes; and original identity, ingestion time, upstream version when available and content hash. Sources distinguish protected Cloud snapshots (`retention: "cloud"`), separately reviewed original documents at stable repository paths (`"repository"`), and provenance-only sources requiring continuing upstream access (`"references"`). Source files are not wiki pages. Immutable version 1 publications remain readable with their original Cloud-snapshot meaning; new Cloud publications write version 2. Raw provider envelopes and executable fixture commands are not manifest fields. Bounds remain 2 MB manifest, 500 files, 1 MB/file, 10 MB combined, 200 knowledge entries, 200 cases and 1,000 sources; planning has the smaller limits above.
...
New version 2 published assignments require `published-artifacts-v2`; the worker also advertises `published-artifacts-v1` for known immutable version 1 assignments and legacy frozen supporting files. Cloud refuses a v1-only worker for a v2 or unverified-schema pin. The worker enforces the assigned `publishedRevision` and explicit `supportingFiles: [{ path, sha256 }]`, including supporting files on legacy staged jobs without a publication pin. It reports the same pin and verified `supportingFileRevisions`; there is no file-only downgrade. Published-checkout verification adds committed-file/fixture scope to, rather than replacing, the existing entry-script binding and separate coverage/repair gates.
