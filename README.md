...
`model` is required for model-assisted planning, uncovered-case exploration, and proposing linked repairs, but not for importing published case definitions, reviewing, or verifying already linked tests/approved repairs. `baseUrl` is required and must match the reviewed plan exactly. It cannot contain credentials, a query, or a fragment. Keep the separate Cloud runner config out of this local config; unknown fields are rejected.
...
- Optional `publishedRevision`: `{ repository, commitSha, manifestPath, manifestSha256 }` pins an exact customer publication, not a deployed build. It is included in the plan approval digest. Imported cases retain `sourceIssues`, script `sha256`, and `history`; these are provenance, not approval.
- Optional `contextSelection`: `{ stages: [{ pageIds, bytes }], omitted: [{ id, reason }] }` records whole-page reading order and unrelated omissions. Required for v3 published plans; review checks its pinned page bytes, source hashes and required detail dependencies. It is included in the approval digest, not an approval itself.
...
### Published customer Git files

Prepare the exact customer checkout yourself, with its `origin` pointing at the selected repository. Toolkit never fetches, switches branches, resets files, or requests Git/Cloud credentials. Import selected existing case definitions without a model:

```sh
npx aratame plan --project . --config aratame.json --repository OWNER/REPO --commit FULL_40_CHARACTER_SHA --manifest aratame/knowledge/manifest.json --knowledge KNOWLEDGE_ID --case CASE_ID
npx aratame review --project . --plan e2e/aratame/plan.json
npx aratame run --project . --config aratame.json --plan e2e/aratame/plan.json --approve REVIEWED_SHA256
```

Repeat `--knowledge` to select explicit IDs; selection expands their surface overview and transitive shared/cross-surface dependencies. An implicitly included overview supplies orientation without expanding unrelated sibling capabilities. Use `--surface NAME` for the complete surface capability map (and cases on that surface); explicit IDs must belong to that surface. Omit both scope flags for the full map. Repeat `--case` to select definitions, or omit it for all in scope. Importing requires 1–40 cases and supports the full 200-page manifest inventory. A KB-only manifest is valid: add `--requirements requirements.txt` and an explicit BYOK model to draft cases. Ordinary Markdown remains usable as an explicitly named `--context` file without Git or a published-revision claim.

The public `published-artifacts.mjs` exports strict `artifactManifestSchema`, `artifactRevisionSchema`, `readPublishedArtifacts({ projectDir, revision })`, and `verifyPublishedCheckout({ projectDir, revision })`, with TypeScript declarations. Both readers return `{ manifest, revision, knowledge, cases, files }`; knowledge/file entries include `content`. `resolvePublishedRevision({ projectDir, repository, commitSha, manifestPath? })` resolves the manifest digest only from the explicit local commit. Repository identity accepts `OWNER/REPO` (GitHub) or a credential-free HTTPS/SSH Git remote. The default manifest is `aratame/knowledge/manifest.json`.

Manifest version 3 (`kind: "aratame-artifacts"`) retains explicit relative files with SHA-256, `kind` (`knowledge`, `conventions`, `provenance`, `script`, `fixture`, `test-definition`, `source-snapshot`), and generated/authored `origin`; knowledge citations, source versions, gaps and history; case definitions and script hashes; and original identity/version/hash. Sources distinguish protected Cloud snapshots (`retention: "cloud"`), separately reviewed original repository documents (`"repository"`), and provenance-only sources requiring continuing upstream access (`"references"`). Source files are not wiki pages. Immutable v1/v2 publications retain their original readers and exact historical pins; new coherent wiki publications use v3. Raw provider envelopes and executable fixture commands are not manifest fields. Bounds remain 2 MB manifest, 500 files, 1 MB/file, 10 MB combined, 200 knowledge entries, 200 cases and 1,000 sources.

V3 generated pages require `wiki: { publicationId, pageKey, role, links }`. `pageKey` is a globally unique stable concept identity; `role` is `overview`, `capability` or `shared`; `links` names required contextual page keys, not filesystem paths. Every generated page belongs exactly once to `publications: [{ id, surface, overviewId, pageIds }]`. Each surface has one overview at `surface::overview`, with its full capability map in `links`; general detail pages use `shared`. Authored pages retain authored ownership and may remain outside these groups without wiki metadata. `conventions: { version: 1, path }` names a conventions-kind document; `indexPath` names a separate conventions/provenance-kind Markdown index linking every current page.

An overview may carry `wiki.evidenceStatus: "withdrawn"` only when it is the sole page in its publication and has empty citations, source versions and dependency links. This marks a deterministic empty-evidence notice, not freshly supported product claims. Authored pages need shared wiki metadata and a declared stable key before a wiki page can link to them as required context; an index may still link ordinary authored pages without that metadata.

The reader checks metadata membership, original citation/revision membership, relative Markdown links and heading anchors, required dependency navigation, and the index. For wiki pages, inline `[SOURCE-ID]` citations must belong to that page's original metadata. Use ordinary inline/reference Markdown links and ATX headings; HTML/plugin navigation is rejected. Fenced code and inline code examples are not navigation. Historical page bytes are retained without being reinterpreted as the current wiki. These are structural guarantees, **not proof that claims are semantically supported**, complete or fresh upstream. Customer conventions and generated pages never become original evidence, platform instructions or execution approval.

`validateArtifactWikiContent(manifest, files)` exposes the same schema/navigation check for a producer's pre-review `{ path, content }` files. It performs no I/O and grants neither verified-checkout status nor original-resolution authority.

`assembleKnowledgeContext(pages, { surface?, knowledgeIds?, maxStageBytes? })` is a pure exported helper with generic TypeScript types. It returns the original selected page objects as `{ knowledge, stages, omitted }`, places overviews before detail, handles dependency cycles, and reports unrelated omissions. General overview orientation and legacy general context remain included. It never fetches originals. Stages pack complete UTF-8 page bodies (default 100,000 bytes); an oversized page or missing required dependency fails explicitly.

Standalone model planning reads those stages in order, builds cumulative coverage, critiques it against every original stage and merges each critique without treating synthesis as evidence authority. Plans retain the complete stage/omission receipt and selected path hashes; unavailable-original warnings remain separate from product gaps. Model review is fallible, especially across stages: inspect cross-page constraints and interactions. Each serialized input stage including metadata/cases is bounded to 150 KB; each request including accumulated draft/critique is bounded to 250 KB, with 100 KB per knowledge page and 2 MB per saved plan. Oversize work fails without saving a partial plan or silently removing constraints. Reorganize a publication or explicitly revise the task rather than dropping required dependencies to fit. Published synthesis and existing cases are context, not self-authorizing tests or original requirements.

`resolvePublishedOriginal(artifacts, sourceId, suppliedOriginal?)` accepts the original result of `readPublishedArtifacts`, not a clone. Repository originals resolve offline from verified pinned files. Cloud and references-only originals report `unavailable` unless you explicitly supply `{ content, upstreamVersion? }`; only matching content hash and recorded upstream version return `exact`, and mismatches return `changed` without historical text. The helper never fetches a URL or requests credentials. A hash detects differences but cannot reconstruct old text. Standalone planning includes source-access metadata and unavailable-original disclosures, not implicit original-body context; choose an ordinary `--context` file explicitly to send original content to your model.

Cloud chooses connector retention and separately reviews repository source disclosure. A later policy change does not erase prior source files or Git history. Compiled wiki prose can itself contain sensitive derived information even without original files. Ordinary local files and standalone commands remain usable without Cloud.

Pinned reads verify `origin`, exact full HEAD, tracked cleanliness (including staged changes/submodules), each declared file against its committed ordinary blob and manifest hash, and reject missing/symlink inputs or hidden/sparse index entries. They also reject untracked **and ignored** overlays, including helper modules and competing Playwright configs; only installed `node_modules` dependency trees are exempt by default. No tracked path is exempt. CLI callers supply exact hash-bound named plan/config/requirements/context/repair-report inputs; explicit local Playwright config/auth-state inputs and selected spec paths are similarly bound. The executor reserves one exclusive current-run `.artifacts/<run>` directory and records individual generated spec hashes, never trusts the whole `e2e/aratame/` directory, and never exempts manifest-pinned paths. Prior unselected outputs are not inferred trustworthy: retain them outside a fresh disposable checkout, or deliberately supply exact local-file allowances through the library API. Dependency installation, external services and an adversarial concurrently changing filesystem remain outside the guarantee.

Library callers may pass `localFiles: [{ path, sha256 }]` for exact operator-selected local inputs, and `outputDirectories` for at most one exclusive current-run `e2e/aratame/.artifacts/<run>` directory. Neither can hide tracked changes or a manifest-pinned file. Do not derive allowances by crawling arbitrary local files or trusting a purported old receipt. Repository hostnames normalize case; only GitHub owner/repository paths are case-insensitive. Other supported Git servers retain repository path case.

Execution checks the pin and all explicitly assigned supporting-file hashes before any fixture/project runtime action and again before certifying results. New operator-reviewed supporting files may supplement, but cannot replace, pinned fixture hashes. A manifest never executes fixtures: only your separate local `fixture` executable/argv setting does. Reports retain `publishedRevision` separately from deployment attribution and include verified `supportingFileRevisions`. Failed preflight is blocked, retains the assigned pin as attribution, and does not invent verified supporting hashes. Changed inputs during execution block certification. Publication does not approve plans, repairs, new assertions or baseline promotion.
...
Version 3 published assignments require `published-artifacts-v3`; the worker also advertises `published-artifacts-v1` and `published-artifacts-v2` for exact historical assignments and legacy frozen supporting files. Cloud must require the assigned manifest version rather than downgrade an unknown pin. The worker enforces the assigned `publishedRevision` and explicit `supportingFiles: [{ path, sha256 }]`, including supporting files on legacy staged jobs without a publication pin. It reports the same pin and verified `supportingFileRevisions`; there is no file-only downgrade. Published-checkout verification adds committed-file/fixture scope to, rather than replacing, existing entry-script binding and separate coverage/repair gates.

Final-candidate assignments may additionally carry an explicitly reviewed `candidateRevision` with the same exact revision shape. The worker verifies `candidateRevision ?? publishedRevision` against the checkout before execution and certification; it never silently executes the accepted HEAD when a candidate was assigned. Reports and local receipts preserve both fields: `publishedRevision` is immutable accepted planning lineage, while `candidateRevision` identifies the proposed checkout actually tested. The latter is not accepted publication, merge approval, baseline promotion, or deployment evidence.
