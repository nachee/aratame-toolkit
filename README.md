...
Manifest version 1 (`kind: "aratame-artifacts"`) lists explicit relative files with SHA-256, `kind` (`knowledge`, `conventions`, `provenance`, `script`, `fixture`, `test-definition`), and generated/authored `origin`. It retains knowledge IDs, surfaces, citations, source versions, gaps and history references; case IDs/versions, behaviors, source issues, linked script hashes and history; and protected Cloud source snapshot references (`retention: "cloud"`). References/hashes cannot reconstruct the original source bytes. Raw source bodies and executable fixture commands are not manifest fields. The manifest excludes itself from its file list. Bounds: 2 MB manifest, 500 files, 1 MB/file, 10 MB combined; 200 knowledge entries, 200 cases, 1,000 source references. Planning has the smaller limits above.
...
Assignments with a published revision or frozen supporting files require `published-artifacts-v1`. The worker enforces the assigned `publishedRevision` and explicit `supportingFiles: [{ path, sha256 }]`, including supporting files on legacy staged jobs without a publication pin. It reports the same pin and verified `supportingFileRevisions`; there is no file-only downgrade. The published-checkout verification described above adds committed-file/fixture scope to, rather than replacing, the existing entry-script binding and separate coverage/repair gates.
